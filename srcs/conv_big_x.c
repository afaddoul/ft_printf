...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/07/08 13:52:30 by afaddoul          #+#    #+#             */
/*   Updated: 2019/07/19 15:06:59 by afaddoul         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char		*create_str_and_fill_struct(t_shape *node, t_conv_x *x)
{
	...
	if (node->flg.flg[4] && node->arg.big_x)
		x->sp -= 2;
	x->sp = (x->sp > 0) ? x->sp : 0;
	return (tmp);
}
...
void			conv_big_x(t_shape *node)
{
	...
	x = NULL;
	x = init_struct_x(x);
	cast_unsigned_type_big_x(node);
	x->hex = ft_itoa_base(node->arg.big_x, 16, 0);
	x->arg_len = ft_strlen(x->hex);
	if (node->p.pre_flg && !node->p.pre && node->arg.big_x == 0)
		x->arg_len = 0;
